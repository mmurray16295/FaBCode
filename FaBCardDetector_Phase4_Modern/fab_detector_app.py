...
class DetectorGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("FaB Card Detector - Control Panel")
            ...
            if class_id < len(names):
                ...
                if self._is_hero_card(card_name):
                    if confidence >= self.hero_detection_threshold:
                        ...
                        # If hero1 is not set (or is this hero), add to hero1 history
                        if not self.hero1_override and (not self.detected_hero1 or self.detected_hero1 == card_name):
                            self.hero1_confidence_history.append((current_time, card_name, confidence))
                        # Otherwise, if this is a different hero, add to hero2 history
                        elif not self.hero2_override and (not self.detected_hero2 or self.detected_hero2 == card_name) and card_name != self.detected_hero1:
