...
if __name__ == "__main__":
    ...
    if existing_total > 0:
        ...
        if args.resume:
            remaining = args.images - existing_total
            if remaining <= 0:
                print(f"\n✓ Already have {existing_total:,} images (target: {args.images:,})")
                sys.exit(0)
            print(f"\nResuming: Will generate {remaining:,} more images")
            ...
            if response in ['yes', 'y']:
                ...
                for split in ['train', 'valid', 'test']:
                    for subdir in ['images', 'labels']:
                        dir_path = Path(f"/root/FaBCode/data/synthetic/{split}/{subdir}")
                        if dir_path.exists():
    ...
    if args.processes is None:
        # Default: use 60% of available cores (conservative)
        args.processes = max(10, int(mp.cpu_count() * 0.6))
        print(f"\nAuto-detected: {args.processes} processes ({mp.cpu_count()} cores available)")
