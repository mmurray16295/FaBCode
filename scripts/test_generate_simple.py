...
def save_yolo_labels(label_path, card_placements, zone_class_mapping, img_width, img_height):
    """Save YOLO format label file with normalized coordinates.
    
    Args:
        label_path: Path to save the .txt label file
        card_placements: List of card placement dicts with x, y, width, height, zone_name
        zone_class_mapping: Dict mapping zone names to class IDs
        img_width: Image width in pixels
        img_height: Image height in pixels
    """
    with open(label_path, 'w') as f:
        for placement in card_placements:
            zone_name = placement.get('zone_name', 'Card')
            class_id = zone_class_mapping.get(zone_name, 4)  # Default to 'Card' class
...
def create_data_yaml(output_dir, force_update=False):
    """Create or update data.yaml file for YOLO training.
    
    Args:
        output_dir: Base directory (e.g., data/synthetic)
        force_update: If True, always recreate the file
    """
    yaml_path = Path(output_dir) / 'data.yaml'
    
    # Check if file already exists and is valid
    if yaml_path.exists() and not force_update:
        print(f"   data.yaml already exists at {yaml_path}")
        return
    
    # Define zone class mapping (must match YOLO training expectations)
    zone_names = [
        'Arms', 'Arms 2', 'Banish', 'Banish 2', 'Card', 'Chest', 'Chest 2',
        'Combat Chain 1', 'Combat Chain 2', 'Graveyard', 'Graveyard 2',
        'Head', 'Head 2', 'Hero', 'Hero 2', 'Legs', 'Legs 2',
        'Pitch', 'Pitch 2', 'Ref', 'Weapon', 'Weapon 2',
        'Weapon or Off-Hand', 'Weapon or Off-Hand 2', 'Window'
    ]
    
    yaml_content = f"""# YOLO Dataset Configuration
# Auto-generated by test_generate_simple.py
...
def main(enable_augmentations=True, draw_bboxes=True, preset_name=None):
    ...
    if zones:
        ...
        card_placements.append({'x': 50, 'y': 50, 'width': hero1_width, 'height': hero1_height, 'label': f"Hero 1: {hero1_card['name']}", 'zone_name': 'Hero'})
        ...
        card_placements.append({'x': 50, 'y': 500, 'width': hero2_width, 'height': hero2_height, 'label': f"Hero 2: {hero2_card['name']}", 'zone_name': 'Hero 2'})
        ...
        for i, (card, img_path) in enumerate(all_card_images):
            ...
            card_placements.append({'x': x, 'y': y, 'width': card_width, 'height': card_height, 'label': card['name'], 'zone_name': 'Card'})
    ...
    # Create zone class mapping (must match data.yaml)
    zone_class_mapping = {
        'Arms': 0, 'Arms 2': 1, 'Banish': 2, 'Banish 2': 3, 'Card': 4,
        'Chest': 5, 'Chest 2': 6, 'Combat Chain 1': 7, 'Combat Chain 2': 8,
        'Graveyard': 9, 'Graveyard 2': 10, 'Head': 11, 'Head 2': 12,
        'Hero': 13, 'Hero 2': 14, 'Legs': 15, 'Legs 2': 16,
        'Pitch': 17, 'Pitch 2': 18, 'Ref': 19, 'Weapon': 20, 'Weapon 2': 21,
        'Weapon or Off-Hand': 22, 'Weapon or Off-Hand 2': 23, 'Window': 24
    }
    
    save_yolo_labels(label_path, card_placements, zone_class_mapping, img_width, img_height)
    print(f"   Saved labels to: {label_path}")
    print(f"   Split: {split} ({len(card_placements)} cards labeled)")
    
    # Create/update data.yaml (only once, subsequent runs will skip)
